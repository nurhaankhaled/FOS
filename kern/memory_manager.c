...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int ret;
	for(uint32 address = virtual_address; address<(virtual_address+size); address +=PAGE_SIZE)
	{
		ret = pf_add_empty_env_page(e, address , 0);
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	struct WorkingSetElement *second_element = NULL;
	...
	LIST_FOREACH(element,&(e->ActiveList))
	{
		if(element->virtual_address >= virtual_address && element->virtual_address < virtual_address+size)
		{
			...
			//if the second list exists and contains elements, insert its head at the tail of the active list
			if(LIST_SIZE(&(e->SecondList)) > 0 && e->SecondListSize > 0)
			{
				second_element = LIST_FIRST(&(e->SecondList));
				LIST_REMOVE(&(e->SecondList),second_element);
				LIST_INSERT_TAIL(&(e->ActiveList),second_element);
				pt_set_page_permissions(e,second_element->virtual_address,PERM_PRESENT,0);
	...
	for(uint32 address = virtual_address; address < (virtual_address+size); address +=PAGE_SIZE)
	{
		...
		//3. Remove ONLY the empty page tables (i.e. not used) (no pages are mapped in the table)
		uint32* ptr_page_table = NULL;
		int ret = get_page_table(e->env_page_directory, (void*)address, &ptr_page_table);
		...
		{
			//check if the page table is empty or not
			int isEmpty = 1;
			...
			if(isEmpty)
			{
				...
				struct Frame_Info *ptr_frame_info = to_frame_info((e->env_page_directory[PDX(address)]>>12)<<12);
				ptr_frame_info->references--;
				if (ptr_frame_info->references == 0)
					free_frame(ptr_frame_info);
