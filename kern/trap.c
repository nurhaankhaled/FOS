...
void allocate_map_frame(struct Env * curenv,uint32 fault_va,struct WorkingSetElement** element);
void table_fault_handler(struct Env * curenv, uint32 fault_va);
void return_to_pf(struct Env* curenv, struct WorkingSetElement* element);
void pf_error_handler(int Error,uint32 fault_va);
bool search_second_list(struct Env* curenv, struct WorkingSetElement* element,uint32 fault_va);
...
void allocate_map_frame(struct Env * curenv,uint32 fault_va,struct WorkingSetElement** element){
	...
	temp = LIST_FIRST(&(curenv->PageWorkingSetList));
	LIST_REMOVE(&(curenv->PageWorkingSetList), temp);
...
void return_to_pf(struct Env* curenv, struct WorkingSetElement* element){
	...
	uint32 pagePerms = pt_get_page_permissions(curenv,element->virtual_address);

	if(pagePerms & PERM_MODIFIED)
	{

		uint32 *ptr_table = NULL;
		struct Frame_Info *ptr_frame_info = get_frame_info(curenv->env_page_directory,(void*)(element->virtual_address),&ptr_table);
		int ret = pf_update_env_page(curenv, (void*)element->virtual_address, ptr_frame_info);
	}
		unmap_frame(curenv->env_page_directory,(void*)element->virtual_address);
		element->empty = 1;
...
void pf_error_handler(int Error,uint32 fault_va){
	if(Error == E_PAGE_NOT_EXIST_IN_PF)
	{
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	//placement
	if( LIST_SIZE(&(curenv->PageWorkingSetList)) > 0 || curenv->SecondListSize == 0)
	{
		if(LIST_SIZE(&(curenv->ActiveList)) == curenv->ActiveListSize)
		{
			last_element = LIST_LAST(&(curenv->ActiveList));
			LIST_REMOVE(&(curenv->ActiveList),last_element);

			//if second list exists
			if(curenv->SecondListSize > 0)
			{
				sec_found = search_second_list(curenv,element,fault_va);
				pt_set_page_permissions(curenv,last_element->virtual_address,0,PERM_PRESENT);
				LIST_INSERT_HEAD(&(curenv->SecondList),last_element);
			}
			else
			{

				//check if if it's modified or not, then return it to the page file
				return_to_pf(curenv,last_element);
			}
		}

		//if not found in second list, remove last element & add the required element
		if(!sec_found)
		{
			//map frame
			allocate_map_frame(curenv,fault_va,&new_element);

			//get info from page file
			int x  = pf_read_env_page(curenv, (void*)fault_va);
			pf_error_handler(x,fault_va);

			LIST_INSERT_HEAD(&(curenv->ActiveList),new_element);
			pt_set_page_permissions(curenv,new_element->virtual_address,PERM_PRESENT,0);
		}
	}
	//replacement
	if(LIST_SIZE(&(curenv->PageWorkingSetList)) == 0 && curenv->SecondListSize > 0)
	{
		//exchange between active and second lists.
		...
		{
			//remove victim from the second list
			element = LIST_LAST(&(curenv->SecondList));
			LIST_REMOVE(&(curenv->SecondList),element);

			//check if if it's modified or not, then return it to the page file
			return_to_pf(curenv,element);

			//remove last element from active list, then insert it at the head of the second list
			LIST_INSERT_HEAD(&(curenv->SecondList),last_element);
			pt_set_page_permissions(curenv,last_element->virtual_address,0,PERM_PRESENT);
